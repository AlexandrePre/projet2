...
const CardModel = ({
  ...
  return [
    <motion.div
      ...
      role="presentation"
    >
      <div className="statsTableRandom">
        <div className="leftColRandom">
          <p id="int">{intelligence} INT</p>
          <p id="str">{strength} STR</p>
          <p id="agi">{speed} AGI</p>
        </div>
        <div className="rightColRandom">
          <p id="sta">{durability} STA</p>
          <p id="fgt">{combat} FGT</p>
          <p id="pwr">{power} PWR</p>
        </div>
      </div>
      <div id="cardPictsRandom">
        <div id="nameRandom">{name}</div>
        <img src={img} alt="" />
        <img id="habillageRandom" src={karma} alt="" />
      </div>
    </motion.div>,
  ];
};
